...
function isTestReceiptRow(row: string[]): boolean {
...
function rowKey(submittedAt: string, merchant: string, date: string): string {
...
async function main(): Promise<void> {
  ...
  const spreadsheetId =
    process.env.GOOGLE_SHEETS_ID ?? "1DnzD0Yn_48ZtD00FyfaGHUMer-7Lle3j6YHu53EAZkg";
  ...
  const toDelete: { sheetRow: number; row: string[] }[] = [];
  const toKeep: { sheetRow: number; row: string[] }[] = [];
  ...
  let ocrDeleteRows: { sheetRow: number; row: string[] }[] = [];
